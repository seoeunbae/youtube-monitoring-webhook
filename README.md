...
**3. Vertex AI (Gemini 2.5 Pro or Flash):**
  역할: 멀티모달(Multimodal) 기능을 활용하여 콘텐츠를 종합적으로 분석.
    분석 내용:
      - 1. 텍스트 분석: Cloud Function에서 전달받은 제목, 설명, 자막 텍스트에 '확률형 아이템 포함' 문구가 있는지 확인.
  영상 분석 (OCR): 동영상 자체를 분석하여, 영상 내에 텍스트(자막, 그래픽 등)로 '확률형 아이템 포함' 문구가 삽입되었는지 프레임별로 광학 문자 인식을 수행.
